def drawHeatmap(PG, hmType, filename, pzero, convertGeneNamesFromType="",
    ...
    if not (len(convertGeneNamesFromType) == 0 or len(convertGeneNamesToType) == 0 or len(
            convertGeneNamesSpecies) == 0):
        columnLabs = convertGeneNames(columnLabs, convertGeneNamesFromType, convertGeneNamesToType,
                                      convertGeneNamesSpecies)
    ...
    # Colorkey:
    if hmType == 'values':
        scale_cbGSSS = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=hm[0, 1], wspace=0.0, hspace=0.0,
        ...
        cb = fig.colorbar(axi, scale_cbAX,
                          orientation='horizontal')  # note that we tell colorbar to use the scale_cbAX axis
