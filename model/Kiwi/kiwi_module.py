...
def plot( gsn, gss, gsc='', gls='', pc=0.01, adj=True, gsz=10**6,
          deg=10**6, spl=2, sae=False, lmp=False, nsz=300, eds=5, lbs=9,
          hmt='binary', cft='', ctt='', csp='', nwf='', hmf='', gml='',
          saveHeatmapMatrix=False,
          heatmapMatrixFile="" ):
    ...
    if dcutoff < max(dict(MN.degree()).values()):
    ...
    for node in PG.nodes():
        PG.nodes[node]['directionalityScore'] = float(color_code[k])
        if nonPiano:
            PG.nodes[node]['-log10p'] = float(p_stable[k,5])
        else:
            PG.nodes[node]['-log10p'] = float(p_stable[k,0])
        k = k+1
    ...
    if len(GMfile)>0 and len(Gfile)>0:
        ...
        kiwiF.drawHeatmap(
            ...
            cgnToType,
            cgnSpecies,
            saveMatrix=saveHeatmapMatrix,
            matrixFilename=heatmapMatrixFile
        )
